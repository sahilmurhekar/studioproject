...
// Ensure DB connection for each request before hitting routes
...
app.get("/api/user-collection/:email", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to fetch user collection" });
  }
});
...
app.post("/api/unsave-beat", async (req, res) => {
  try {
    ...
    await db
      .collection("user_collections")
      .updateOne({ email }, { $pull: { savedBeats: beatId } });

    res
      .status(200)
      .json({ success: true, message: "Beat removed from collection" });
  } catch (error) {
    console.error("Error removing beat:", error);
    res
      .status(500)
      .json({ success: false, message: "Failed to remove beat" });
  }
});
...
app.post("/api/purchase-beat", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to purchase beat" });
  }
});
